...
size_t (*lame_read_sample_buffer)(void *buffer, size_t size,  size_t count, FILE *stream , char* pData, int dataLen );
...
static int read_samples_pcm(FILE * musicin, int* sample_buffer/*[2304]*/, int samples_to_read, char* pData, int dataLen, int num_channels);
...
static int
        get_audio_common(lame_t gfp, int buffer[2][1152], short buffer16[2][1152], char* pData, int dataLen);
...
************************************************************************/
int
get_audio(lame_t gfp, int buffer[2][1152], char* pData, int dataLen)
{
    int     used = 0, read = 0;
    do {
        read = get_audio_common(gfp, buffer, NULL, pData, dataLen);
...
get_audio16(lame_t gfp, short buffer[2][1152])
{
    int     used = 0, read = 0;
    do {
        read = get_audio_common(gfp, NULL, buffer, NULL, 0);
...
get_audio_common(lame_t gfp, int buffer[2][1152], short buffer16[2][1152], char* pData, int dataLen)
{
    ...
    if (is_mpeg_file_format(global_reader.input_format)) {
        ...
        if (global.snd_file) {
            ...
            samples_read =
                read_samples_pcm(global.music_in, insamp, num_channels * samples_to_read, pData, dataLen, num_channels);
        }
        if (samples_read < 0) {
            return samples_read;
        }
...
extern SNDFILE *sf_wchar_open(TCHAR const *wpath, int mode, SF_INFO * sfinfo);
...
open_snd_file(lame_t gfp, char const *inPath)
{
    ...
    {
#ifdef _WIN32
        TCHAR *file_name = utf8ToUnicode(lpszFileName);
#endif
...
unpack_read_samples(const int samples_to_read, const int bytes_per_sample,
                    const int swap_order, int *sample_buffer, FILE * pcm_in, char* pData, int dataLen)
{
    size_t  samples_read;
    int     i;
...
#define GA_URS_IFLOOP( ga_urs_bps ) \
	...
	samples_read = lame_read_sample_buffer(sample_buffer, bytes_per_sample, samples_to_read, pcm_in, pData, dataLen);
...
static int
read_samples_pcm(FILE * musicin, int* sample_buffer/*[2304]*/, int samples_to_read, char* pData, int dataLen,int num_channels)
{
	int     samples_read;
    int     bytes_per_sample = (global.pcmbitwidth / 8);// * num_channels / 2 ;
    ...
    samples_read = unpack_read_samples(samples_to_read, bytes_per_sample, swap_byte_order,
                                       sample_buffer, musicin, pData, dataLen);
    if(samples_read == -1)// ferror(musicin)) 
	{
        if (global_ui_config.silent < 10) {
...
open_wave_file(lame_t gfp, char const *inPath)
{
    ...
    if (!strcmp(inPath, "-")) {
        ...
        if ((musicin = /*lame_*/fopen(inPath, "rb")) == NULL) {
            if (global_ui_config.silent < 10) {
